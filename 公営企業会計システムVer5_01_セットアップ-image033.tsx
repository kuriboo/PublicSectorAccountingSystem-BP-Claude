...
このコンポーネントでは、水道料金の計算に必要な情報を入力するフォームと、計算結果を表示するテーブルを備えています。フォームの入力値はstateで管理され、サブミット時にonSubmitコールバックを介して親コンポーネントに渡されます。

スタイリングにはstyled-componentsを使用し、レスポンシブデザインを考慮してコンポーネントを配置しています。また、入力値のバリデーションや例外処理も一部含まれています。

サンプルデータを用いた使用例も同じファイル内に含めています。これにより、コンポーネントの再利用性を確保しつつ、実際の使用方法がわかりやすくなっています。