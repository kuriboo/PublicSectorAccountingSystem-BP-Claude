以下がNext.js + TypeScriptで実装した支払伝票コンポーネントです。
...
このコンポーネントは、渡されたプロパティに基づいて支払伝票を表示します。スタイリングにはstyled-componentsを使用し、CSS-in-JS形式でコンポーネント内にスタイルを定義しています。

また、`SamplePaymentSlip`コンポーネントでは、サンプルデータを使用して`PaymentSlip`コンポーネントを表示しています。

コンポーネントは再利用可能で、プロパティを通じてカスタマイズ可能です。例外処理として、プロパティが渡されない場合は空文字列や0が代入されるようにデフォルト値を設定しています。

レスポンシブデザインについては、グリッドレイアウトやフレックスボックスを使用することで、画面サイズに応じた表示の調整が可能です。

コメントを適切に挿入し、コードの理解を助けるようにしています。

このコンポーネントを使用することで、支払伝票の表示を簡単に実装できます。